...
export default function NotificationDropdown() {
  ...
  const loadUnread = () => {
    ...
    notificationsApi.getUnread(user.userId).then((list) => {
      setNotifications(list);
      setUnreadCount(list.length);
    });
  };

  useEffect(loadUnread, [user?.userId]);
  ...
  return (
    <div className="relative">
      <button
        className="relative flex items-center justify-center text-indigo-100 transition-colors bg-white/10 border border-white/20 rounded-full dropdown-toggle h-11 w-11 hover:bg-white/20 hover:text-white"
        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          ...
          {notifications.map((n) => (
            <li key={n.notificationId}>
              <DropdownItem
                onItemClick={() => handleItemClick(n.notificationId)}
